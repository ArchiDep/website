...
The slides' and cheatsheets' PDFs are generated by a separate step — a CI job,
and the same commands by hand. What each one is called is decided here,
by [`PdfNames`](./build/pdf_names.ex), because it follows from the page it is
of; _where_ they are published is the deployment fact this subsystem is told
rather than deciding. [`PdfManifest`](./urls/pdf_manifest.ex) carries both, as a
base — `:site` (under the build's own prefix) or `{:external, base}` (a bucket,
a release — anywhere the server does not have to store them) — and an entry per
page. An entry may also be a `{:url, url}` override, for a host that renames
what it is given and therefore publishes at a URL that cannot be derived from
the local file name.

The entries are worked out by [`Builder`](./builder.ex) from the course it read,
the way the two asset manifests are, so the caller states the base and nothing
else: `:pdf_base` on `Builder.build/1`, `--pdf-base` on the build task, or
`pdf_base` in the `course_site` configuration. **A build told nothing publishes
nothing** — an empty manifest, and no page offering a download. That is what a
build made before its PDFs have been printed wants, and it is why an empty
manifest is a valid build rather than a misconfiguration. Note that a `:site`
base says where the PDFs are, not that the build writes them: nothing here
produces a PDF.
...
leave the download link out. Automating the export does not change that, since
