defmodule ArchiDep.CourseSite.Layout.Chrome.Policy do
  ...
  wherever it is served from. A few pieces are not, because they point at the
  running application — signing in, the account menu, the switch between the
  course and the dashboard, the badges saying whether the site is up. A build
  that is not the live site either cannot reach those or should not offer them,
  ...
  a build, and only happen to agree today.
  """

  alias ArchiDep.CourseSite.Urls.UrlContext

  @enforce_keys [:app_navigation?, :account?, :badges?]
  defstruct [:app_navigation?, :account?, :badges?]

  @typedoc """
  What the chrome of a build draws of the running application: the menu
  switching between the course and the dashboard, the ways in and out of an
  account, and the badges reporting on the site itself.
  """
  @type t :: %__MODULE__{
  ...
  Work out what the chrome of a build carries from where it is published.
      ...
      %Policy{app_navigation?: true, account?: true, badges?: true}

      iex> Policy.of(UrlContext.new(mode: :backup, build_id: "build"))
      %Policy{app_navigation?: false, account?: false, badges?: false}
  ...
  def of(%UrlContext{mode: mode}) do
    ...
    %__MODULE__{app_navigation?: live?, account?: live?, badges?: live?}
  end
end
