defmodule ArchiDep.Accounts.Behaviour do
  ...
  is registered.
  """
  @callback log_in_or_register_with_switch_edu_id(Types.switch_edu_id_login_data(), map) ::
              {:ok, Authentication.t()}
  ...
  """
  @callback log_in_or_register_with_link(binary(), map) ::
  ...
  an active session.
  """
  @callback validate_session_token(String.t(), map) ::
  ...
  session.
  """
  @callback validate_session_id(UUID.t(), map) ::
              {:ok, Authentication.t()} | {:error, :session_not_found}

  @doc """
  Returns the list of active sessions for the currently authenticated user.
  """
  @callback fetch_active_sessions(Authentication.t()) :: list(UserSession.t())
  ...
  """
  @callback impersonate(Authentication.t(), UUID.t()) ::
  ...
  """
  @callback stop_impersonating(Authentication.t()) :: :ok | {:error, :unauthorized}

  @doc """
  Deletes the specified session.
  """
  @callback delete_session(Authentication.t(), String.t()) ::
              {:ok, UserSession.t()} | {:error, :session_not_found}

  @doc """
  Fetches the user account corresponding to the current authentication.
  """
  @callback user_account(Authentication.t()) :: UserAccount.t()

  @doc """
  Logs out a user account.
  """
  @callback log_out(Authentication.t()) :: :ok | {:error, :session_not_found}

  @doc """
  Creates a new login link for the specified preregistered user.
  """
  @callback create_login_link_for_preregistered_user(Authentication.t(), UUID.t()) ::
              {:ok, LoginLink.t()}
