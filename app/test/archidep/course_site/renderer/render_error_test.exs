defmodule ArchiDep.CourseSite.Renderer.RenderErrorTest do
  ...
  describe "message/1" do
    test "says what is wrong and where, for every kind of problem" do
      assert Enum.map(
               ...
               &Exception.message(RenderError.new(&1, "chapters/507-dns/subject.md"))
             ) == [
               "Invalid front matter (expected a mapping, got []) in chapters/507-dns/subject.md",
               "The front matter is never closed by a line of three dashes in chapters/507-dns/subject.md",
    ...
    test "says where in the file the problem is when it knows" do
      error =
        RenderError.new({:liquid, "Boom"}, "chapters/507-dns/subject.md", %{line: 12, column: 3})

      assert Exception.message(error) ==
               "Boom in chapters/507-dns/subject.md at line 12, column 3"
    end
  end
  ...
  describe "shift/2" do
    test "leaves an error that is not anywhere in particular alone" do
      error = RenderError.new({:liquid, "Boom"}, "chapters/507-dns/subject.md")
