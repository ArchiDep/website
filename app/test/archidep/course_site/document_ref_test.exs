defmodule ArchiDep.CourseSite.DocumentRefTest do
  ...
  describe "parse_source_path/1" do
    test "parses a subject" do
      assert DocumentRef.parse_source_path("chapters/104-ssh/subject.md") ==
               {:ok, %DocumentRef{num: 104, slug: "ssh", type: :subject}}
    end

    test "parses an exercise" do
      assert DocumentRef.parse_source_path("chapters/205-php-todolist/exercise.md") ==
               {:ok, %DocumentRef{num: 205, slug: "php-todolist", type: :exercise}}
    end

    test "parses slides at the root of a chapter" do
      assert DocumentRef.parse_source_path("chapters/401-cloud-computing/slides.md") ==
    ...
    test "parses slides in a subdirectory as the same kind of document" do
      assert DocumentRef.parse_source_path("chapters/202-git-branching/slides/slides.md") ==
               {:ok, %DocumentRef{num: 202, slug: "git-branching", type: :slides}}
    end

    test "rejects an unknown document type" do
      assert DocumentRef.parse_source_path("chapters/301-nginx/README.md") ==
               {:error, {:invalid_source_path, "chapters/301-nginx/README.md"}}
    end

    test "rejects a chapter number outside a section" do
      assert DocumentRef.parse_source_path("chapters/042-history/subject.md") ==
               {:error, {:invalid_source_path, "chapters/042-history/subject.md"}}
    end

    test "rejects a cheatsheet" do
      assert DocumentRef.parse_source_path("cheatsheets/git/cheatsheet.md") ==
               {:error, {:invalid_source_path, "cheatsheets/git/cheatsheet.md"}}
    end

    test "rejects slides nested deeper than one directory" do
      assert DocumentRef.parse_source_path("chapters/601-ci/slides/parts/slides.md") ==
               {:error, {:invalid_source_path, "chapters/601-ci/slides/parts/slides.md"}}
    end
  end
end
