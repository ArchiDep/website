defmodule ArchiDep.Servers.Schemas.ServerOwnerCountersTest do
  ...
  describe "initial_changeset/2" do
    test "creates the counters row for an owner's first server in a class" do
      %{id: user_account_id} = AccountsFactory.insert(:user_account, root: true)
      %{id: class_id} = CourseFactory.insert(:class)

      inserted = Repo.insert!(ServerOwnerCounters.initial_changeset(user_account_id, class_id))
  ...
  describe "database constraints" do
    test "rejects a negative active-server count" do
      ...
      assert_raise Ecto.ConstraintError, ~r/active_server_count_is_not_negative/, fn ->
        insert_counters!(user_account_id, class_id, server_count: 0, active_server_count: -1)
      end
    end
    ...
    test "rejects an active count greater than the server count" do
      ...
      assert_raise Ecto.ConstraintError,
                   ...
                   fn ->
                     insert_counters!(user_account_id, class_id,
                       server_count: 0,
  ...
  defp insert_counters!(user_account_id, class_id, attrs) do
