defmodule ArchiDep.Servers.Ansible.Pipeline.AnsiblePipelineRunnerTest do
  ...
  describe "process_event/1 running a playbook" do
    ...
    test "starts the run and streams its events when the server is online", %{server: server} do
      ...
      event = playbook_event("Install packages")
  ...
  # A type-valid `AnsiblePlaybookEvent` for the mocked Ansible stream: the
  # runner only reads `task_name`, but the `ServerManagerClient` contract checks
  # the whole struct, so `run` is left unloaded (valid per its type) rather than
  # carrying a half-loaded association.
  defp playbook_event(task_name),
    do: %AnsiblePlaybookEvent{
      id: UUID.generate(),
