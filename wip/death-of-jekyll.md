...
- [Task details](#task-details)
  ...
  - [Optional URL prefix](#optional-url-prefix)
    ...
    - [Where standing the repository up has got to](#where-standing-the-repository-up-has-got-to)
  ...
  - [The build is not byte-reproducible](#the-build-is-not-byte-reproducible)
  - [Cutover](#cutover)
...
- [x] Decide where per-year generated PDFs are kept alongside the archived site
      — see [Per-year PDF archive](#per-year-pdf-archive). **Decided: release
      assets of the [archive repository](#where-past-editions-are-kept), one
      release per edition tagged `pdf/<year>`, clobbered during the year and
      never written to again after it.** A release asset is outside the git
      history, the repository size and the published tree, so the 127 MB a year
      of blobs that never repeat cannot swamp the one artifact meant to stay
...
Which of the two a build gets is now settled — `{:external, base}`, pointing at
a release of the archive repository — but that is a deployment fact the seam is
built to be indifferent to, and the `:site` base stays supported. See [Per-year
PDF archive](#per-year-pdf-archive).
...
2. Commit it as `<year>/` in the archive repository. The year's PDFs are **not**
   part of that tree; they are a release of the same repository, and the
   rollover does not touch them — see [Per-year PDF
...
**kept alongside the archived static site** for that year, so that an archived
`/2025/` site and its PDFs travel together. Not strictly part of the rendering
refactor, but a stated goal.

**Decision: they are release assets of the [archive
repository](#where-past-editions-are-kept), one release per edition, tagged
`pdf/<year>` and written by CI.** They travel with the archived site — same
repository, same account, same credential — without being _in_ it.

**A release asset is not repository content**, and that distinction is the whole
reason this works. It sits outside the git history, outside the repository size
and outside the published Pages tree, so the objection that keeps the PDFs out
of the archived tree — 127 MB an edition across 65 files, none of it shared with
the year before, in the one artifact meant to stay cheap to clone and to stay
under the host's published-size ceiling — does not reach a release hanging off
that same repository. A clone stays the ~53 MB of an edition while the PDFs sit
beside it, free, permanently addressable and downloadable without an account.
Individual assets are capped at 2 GB, two orders of magnitude above the largest
file here, and release downloads are not metered the way Git LFS's quota is.

**Overwriting during the year and freezing after it need no distinct steps.**
Each generation run uploads with `--clobber`, replacing the assets in place at
unchanged URLs, so the live edition's PDFs are always the current ones. At the
[year-end rollover](#where-past-editions-are-kept) nothing moves and nothing is
copied: the next edition opens its own release and the previous one is simply
never written to again. That matters more than the convenience — an archive that
becomes permanent only when somebody remembers to make it permanent is an
archive that will eventually be missing a year.

**It adds no account and no credential.** Publishing the frozen editions already
costs one token that can push into the archive repository; the same token
creates releases there. That is the criterion the site's own storage decision
turned on — object storage was rejected as "one more account that has to outlive
the course" — and the PDFs are held to it too, because a past edition's PDFs
have to survive the app, the domain and eventually this repository.

**CI sanitises the asset names on the way up.** GitHub rewrites spaces and other
characters in an uploaded name into periods, and today's exports are named
`ArchiDep 000 - Course.pdf` and `ArchiDep 999 - <title>.pdf` — so the host would
be choosing the URL. Better that the publish step choose it, uploading flat
lowercase names (`archidep-000-course.pdf`): the URL is then derivable from
`{:external, base}` alone, and the per-entry `{:url, …}` override of
[`PdfManifest`](#generated-pdfs-may-live-anywhere) goes back to being the safety
...
downloads). It loses on the account criterion above, not on capability, and it
is the obvious fallback if the release-asset route is ever ruled out.

Rejected outright: a **separate Pages repository** for the PDFs, because
overwriting compressed blobs through a semester bloats a history that cannot
delta-compress them, and the 1 GB published-size ceiling arrives within a
decade; **Git LFS**, whose free tier is 1 GB of storage _and_ 1 GB of bandwidth
a month; and **regenerating** a past year's PDFs on demand from its frozen HTML,
which trades stored bytes for a dependency on a browser whose rendering drifts —
the same reason [a finished edition is frozen output rather than something
rebuilt](#where-past-editions-are-kept).

Publication is **CI's** either way, since that is where generation and
publication end up once [the export no longer needs the production
