...
- the course material site implemented with Jekyll (see [`CONTRIBUTING.md` in
  the `course` directory][course-contributing]).
...
- **Styling Guidelines**
  - Prefer Tailwind utility classes. Use the `@apply` directive to attach
    utilities to semantic selectors when styling markup generated by Jekyll,
    Rouge or third-party libraries that you cannot annotate with classes
    directly.
...
[Rouge][rouge] (the highlighter used by Jekyll), which emits `.highlight` markup
with token classes (`.k`, `.s`, `.nf`, etc.). The theme provides two color
schemes for these classes:

- `highlight-light.css` is the default scheme, based on the Solarized Light
  palette.
- `highlight-dark.css` wraps a complementary high-contrast dark scheme in a
  `@media (prefers-color-scheme: dark)` query, so highlighting follows the
  system color scheme to match the [DaisyUI dark theme](#themes--dark-mode).
...
- For a one-off build, run `npm run --workspace theme build` from the repository
  ...
  [README.md](../README.md).
- For development, run the theme in watch mode with `npm start` (from the `theme`
  directory) so that the stylesheets are rebuilt whenever a source stylesheet or
  a scanned template changes. The watch task runs alongside the application and
  Jekyll, as described in the [development instructions](../README.md). When
  using the Docker development environment, this runs automatically in the
  `theme` container.
...
- [Rouge][rouge] for syntax highlighting (via Jekyll)
