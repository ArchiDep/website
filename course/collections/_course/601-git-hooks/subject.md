...
- [Git Introduction]({% link _course/201-git/subject.md %})
...
These example files are not triggered because they are not executable and do not
have the right name.
...
You can type the `git help hooks` command to get the full list of hooks and
their documentation. A full list is also available [here][git-hooks].
...
Create a hook script named `pre-commit` that will be executed before every
commit:
...
Note that hooks are not part of the working tree and thus **cannot be put under
version control**:
...
This is a good thing, as you don't want people to be able to send malicious hook
scripts to your machine every time you clone a repository from the Internet.

{% endnote %}

### How do I use a hook?

Following the previous example, make a commit and the `pre-commit` hook will be
...
Hooks can be a way to automate repetitive or mundane tasks, like removing
trailing spaces at each commit.

Or they can be used to automate deployments, for example to run the latest
version of a server-side application every time new commits are pushed.
