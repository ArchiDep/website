...
To protect yourself from [man-in-the-middle attacks][man-in-the-middle-attacks], you can obtain the SSH host key fingerprints from your virtual machine before attempting to
...
{% callout type: more, id: azure-no-password %}
...
{% note type: warning %}
:warning: You should not use underscores (`_`) in a hostname, use hyphens (`-`) instead.
{% endnote %}
...
{% callout type: warning %}
The cloud servers used in this course do not have enough memory (RAM)
...
{% note type: tip %}
You can also see the currently available swap space and how much is used with the `htop` command which shows it as the `Swp` bar at the top (you can quit it with `q` once it is open). For more information, see the [fstab Linux manpage][fstab-linux-manpage].
{% endnote %}

## :exclamation: Add your Azure VM to archidep.ch
...
Azure requires that [SSH keys of type RSA have at least 2048 bits][azure-ssh-key-formats].
