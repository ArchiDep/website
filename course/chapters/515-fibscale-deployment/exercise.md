---
title: Horizontally scale a web application with nginx as a load balancer
...
chapters/509-reverse-proxy/subject.md %}), that you have nginx installed and
running on a server, and that you have a DNS wildcard entry preconfigured to
make various subdomains (e.g. `*.jde.archidep.ch` in this guide) point to that
server.
...
The application you will deploy is [FibScale][fibscale], a web application that
computes [Fibonacci numbers][fib].
...
You can check that everything has been correctly installed with the following
commands:
...
Bundler is Ruby's package manager much like Composer for PHP or npm
for Node.js.
...
Replace `jde` with your username in the `WorkingDirectory` and `User` options.
...
- The [iterative
  algorithm](https://github.com/ArchiDep/fibscale/blob/d62a2c5015202f1097c7995b2463c7b498c6827d/fibscale.rb#L153-L163).
...
{% callout type: more, id: fibscale-why-not-high-fib %}

Why not compute a very high Fibonacci number instead of adding a fake delay? If
you have played with FibScale, you may have noticed that the higher the
...
Because scaling an application is a complex task and you may not do it the same
way depending on the cause of the slowdown (e.g. CPU-bound or I/O-bound). If we
slow down the computation by consuming more CPU (i.e. computing a high Fibonacci
number), we may hit the limits of our server's CPU(s) too quickly. By
artificially slowing down computation without consuming more CPU or other
resources, we can simplify the demonstration and concentrate on configuring load
...
Do not stray too far from the instructions below when load testing when it comes
to the number of users. You are going to simulate many users sending requests to
your application concurrently. If you simulate too many users, you may bring
down your server, or your test may easily be misconstrued as a [DoS attack][dos]
by the Azure cloud. Your server may end up being blacklisted.
...
These assumptions do not necessarily represent a real-world scenario, but since
they are true for this exercise, it will allow us to perform scaling on our
single server.
...
integer to the application will change the color of its navbar to help identify
different instances.
...
## :exclamation: Not the solution to all your problems
...
{% callout type: more, id: fibscale-performance %}
...
Again, **be careful of issues such as [rate limiting][rate-limiting] and
[(D)DoS][dos] protection when load testing**. Load tests can look a lot like a
DoS attack. You may inadvertently be banned by your cloud provider if you're not
careful.
...
- It consumes little CPU (when using the iterative algorithm) and is neither
  ...
  function](https://github.com/ArchiDep/fibscale/blob/00dba541f53468c94b239b692372a714f5b919e1/fibscale.rb#L84).
  This consumes neither CPU nor memory, and does not perform any I/O.
- It is [pre-configured to be
  single-threaded](https://github.com/ArchiDep/fibscale/blob/00dba541f53468c94b239b692372a714f5b919e1/config/puma.rb#L4-L5)
...
<div class="flex items-center gap-2">
  <a href="./images/architecture.pdf" download="FibScale Deployment Architecture" class="tooltip" data-tip="Download PDF">
    {%- include icons/document-arrow-down.html class="size-12 opacity-50 hover:opacity-100" -%}
  </a>
  <a href="./images/architecture.png" download="FibScale Deployment Architecture" class="tooltip" data-tip="Download PNG">
...
[fibscale]: https://github.com/ArchiDep/fibscale
[fibscale-config]: https://github.com/ArchiDep/fibscale#configuration
[fibscale-locustfile]: https://github.com/ArchiDep/fibscale/blob/main/locustfile.py
