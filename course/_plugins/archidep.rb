  ...
  class Generator < Jekyll::Generator
    def generate(site)
      ...
      course_docs.each do |item|
        m = /\/([1-9])(\d\d)-([^\/]+)\/(exercise.md|slides.md|slides\/slides.md|subject.md)\z/.match(item.path)
        throw %/Invalid filename format for item: "#{item.path}"/ unless m

        section = Integer(m[1])
        section_chapter = Integer(m[2])
        slug = m[3]
        course_type = if m[4] == 'slides/slides.md'
          'slides'
        else
          m[4].sub(/\.md\z/, '')
        end
      ...
      course_docs.each do |doc|
        if doc.data['course_type'] == 'subject'
          num = doc.data['num']
          slides_doc = course_docs.filter{ |item| item.data['num'] == num && item.data['course_type'] == 'slides' }
      ...
      sections.each_with_index do |section,i|
        ...
        section_docs = course_docs.select{ |item| item.data['section'] == i + 1 }
        section['items'] = section_docs.reject{ |item| item.data['course_type'] == 'slides' && course_docs.any?{ |subject| subject.data['course_type'] == 'subject' && subject.data['num'] == item.data['num'] } }
      ...
      previous_chapters = course_docs.select{ |doc| doc.data['course_type'] == 'subject' && last_done.include?(doc.data['num']) }
      home_page_doc.data['previous_chapters'] = previous_chapters

      last_due = progress_docs.reverse.find{ |doc| doc.data['due'] }&.data&.[]('due') || []
      next_due_exercises = course_docs.select{ |doc| doc.data['course_type'] == 'exercise' && last_due.include?(doc.data['num']) }
      ...
      next_chapters = course_docs.select{ |doc| doc.data['subject'] == nil && next_chapter_nums.include?(doc.data['num']) }
