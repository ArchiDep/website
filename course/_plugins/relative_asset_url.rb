module ArchiDep
  module RelativeAssetUrlFilter
    ...
    def relative_asset_url(path)
      ...
      result = if asset_dir == course_dir
        course_dir_path = Pathname::new(course_dir)
        determine_course_asset_url(Pathname::new(asset_file).relative_path_from(course_dir_path), webpack_manifest_file, dest_dir_path)
      else
        determine_other_asset_url(Pathname::new(asset_file).relative_path_from(dest_dir_path), phoenix_cache_manifest_file)
      end
    ...
    def determine_course_asset_url(relative_path, webpack_manifest_file, dest_dir_path)
      manifest = JSON.parse(File.read(webpack_manifest_file))
      result = manifest["/assets/course/#{relative_path.to_s}"]
      raise "Course asset #{relative_path.to_s.inspect} not found in manifest #{webpack_manifest_file.inspect}" unless result
    ...
    def determine_other_asset_url(relative_path, phoenix_cache_manifest_file)
      if Jekyll.env == "production"
        raise "Manifest file #{phoenix_cache_manifest_file.to_s.inspect} does not exist" unless File.exist?(phoenix_cache_manifest_file)
      elsif not File.exist?(phoenix_cache_manifest_file)
